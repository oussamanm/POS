...
class ProductsController extends BaseController
{
    ...
    public function index(request $request)
    {
        ...
        foreach ($products as $product) {
            ...
            if($product->type == 'is_single')
            {
              ...
              $item['quantity'] = $product_warehouse_total_qty;
    ...
    public function get_products_return(request $request)
    {
        //get warehouses assigned to user
        $user_auth = auth()->user();
        if($user_auth->is_all_warehouses){
        ...
        if (isset($_GET['type_return']) && $_GET['type_return'] == 'sale')
        {
            $clients = Client::where('deleted_at', '=', null)->get(['id', 'name']);
            return response()->json([
                'clients' => $clients,
        ...
        else if (isset($_GET['type_return']) && $_GET['type_return'] == 'purchase')
        {
    ...
    public function app_index(request $request)
    {
        ...
        // Filter fields
        if (empty($request->warehouseId) || $request->warehouseId === -1)
        {
            if (!empty($current_user->warehouse) && $current_user->warehouse !== null)
                $warehouse_id = $current_user->warehouse;
            else
                $warehouse_id = 1;
        }
        else
        ...
        // Filter
        if (!empty($request->categoryId) && is_numeric($request->categoryId))
            $products = $products->where('category_id', $request->categoryId);

        // Search
        if (!empty($request->search) && $request->search !== "")
        {
            $search_value = strtolower($request->search);

            $products = $products->where(function($query) use ($search_value){
                $query->where('code', 'LIKE', "%{$search_value}%")
                    ->orWhere('name', 'LIKE', "%{$search_value}%")
                    ->orWhere('ar_name', 'LIKE', "%{$search_value}%");
            });
        }

        $products = $products
            ->leftJoin('product_warehouse',  function($join) use($warehouse_id) {
                    $join->on('product_warehouse.product_id', '=', 'products.id')
                        ->where('product_warehouse.deleted_at', '=', null)
                        ->where('warehouse_id', $warehouse_id);
            })
            ->select(['products.id','products.category_id', 'unit_id', 'brand_id', 'products.code', 'products.name', 'ar_name', 'image', 'cost', 'price', 'min_price', 'tax_method', 'TaxNet', 'unit_purchase_id', DB::raw('SUM(qte) as qty')])
            ->groupBy('products.id')
            ->havingRaw('qty > 0');
    ...
    public function products_warehouse(request $request)
    {
        $this->authorizeForUser($request->user('api'), 'view', Product::class);
        // How many items do you want to display.
        ...
        if (!empty($current_user) && $current_user->warehouse !== null)
            $warehouse_id = $current_user->warehouse;
    ...
    public function products_return(request $request)
    {
        $this->authorizeForUser($request->user('api'), 'view', Product::class);
        // How many items do you want to display.
        ...
        // Filter Sale of return
        if (!empty($request->saleId) && is_numeric($request->saleId))
        {
            // get products of sale id
            $products_sale =  SaleDetail::where('sale_id', $request->saleId)
                ->leftJoin('products', 'sale_details.product_id', '=', 'products.id')
                ->select(['products.id','products.category_id', 'products.unit_id', 'products.brand_id', 'products.code', 'products.name', 'ar_name', 'image', 'products.cost', 'products.price', 'products.min_price', 'products.tax_method', 'products.TaxNet', 'products.unit_purchase_id', 'sale_details.quantity as sale_detail_qty', 'sale_details.price as sale_detail_price', 'sale_details.discount_method as sale_detail_discount_method', 'sale_details.discount as sale_detail_discount'])
                ->paginate($limit);
        ...
        $products = $products->select(['products.id','products.category_id', 'unit_id', 'brand_id', 'products.code', 'products.name', 'ar_name', 'image', 'cost', 'price', 'min_price', 'tax_method', 'TaxNet', 'unit_purchase_id'])
        ->paginate($limit);
    ...
    public function products_warehouse_depot(request $request)
    {
        $this->authorizeForUser($request->user('api'), 'view', Product::class);
        // How many items do you want to display.
        ...
        $products = $products
            ...
            ->select(['products.id','products.category_id', 'unit_id', 'brand_id', 'products.code', 'products.name', 'ar_name', 'image', 'cost', 'price', 'tax_method', 'TaxNet', 'unit_purchase_id', DB::raw('SUM(qte) as qte')])
            ->groupBy('products.id')
            ->havingRaw('qte > 0')
            ->paginate($limit);
    ...
    public function products_warehouse_exchange(request $request)
    {
        $this->authorizeForUser($request->user('api'), 'view', Product::class);
        ...
        $products = Product::with('unit:id,ShortName,name', 'category:id,code,name', 'brand:id,name')->where('products.deleted_at', '=', null);

        // Get user warehouse
        $user_id = auth()->user()->id;
        $warehouse_user = UserWarehouse::where('warehouse_id', '!=', 1)
            ->where('user_id', $user_id)
            ->pluck('warehouse_id')->first();

        if (empty($warehouse_user) || is_null($warehouse_user))
            $warehouses_id = 1;
        else
            $warehouses_id = $warehouse_user;
        ...
        $products = $products
            ->leftJoin('product_warehouse',  function($join) use($warehouses_id) {
                    $join->on('product_warehouse.product_id', '=', 'products.id')
                        ->where('product_warehouse.deleted_at', '=', null)
                        ->where('warehouse_id', $warehouses_id);
            })
            ->select(['products.id','products.category_id', 'unit_id', 'brand_id', 'products.code', 'products.name', 'ar_name', 'image', 'cost', 'price', 'tax_method', 'TaxNet', 'unit_purchase_id', DB::raw('SUM(qte) as qty')])
            ->groupBy('products.id')
            ->havingRaw('qty > 0')
            ->paginate($limit);
        ...
        // $products = $products->orderBy($order, $dir)->paginate(15);
        ...
        //         $item['quantity'] = $product_warehouse_total_qty .' '.$product['unit']->ShortName;
        //         $item['qte'] = $product_warehouse_total_qty;
        //     }


        //     if ($product_warehouse_total_qty == 0)
        //         continue;
    ...
    public function store(Request $request)
    {
        ...
        {
            $request['tax_method'] = "2";
            $m = 1 + $request['TaxNet'] / 100;
            $request['price'] *= $m;
        }
        try {
    ...
    public function Get_Products_Details(Request $request, $id)
    {
        ...
        /// the first condition is to display for vendeur just there Warehouse
        // if ($user_auth->role_id === 2)
        // {
        //     $warehouses_id = UserWarehouse::where('user_id', $user_auth->id)->where('warehouse_id', '<>', 1)->pluck('warehouse_id')->toArray();
        //     $warehouses = Warehouse::where('deleted_at', '=', null)->whereIn('id', $warehouses_id)->get(['id', 'name']);
        // }
        // else
        if($user_auth->is_all_warehouses)
        ...
        $item['tax_method'] = $Product->tax_method == '1' ? 'HT' : 'TTC';
    ...
    public function get_filter_returns($id_client)
    {
        //get warehouses assigned to user
        if (empty($id_client) || $id_client === -1)
        {
            return response()->json([
                'status'=>  false,
                'message' =>  'invalid input, id_client should be a valid number'
            ]);
        }

        $sales = Sale::where('client_id', $id_client)->where('deleted_at', '=', null)->select(['id', 'Ref', 'GrandTotal', 'date'])->get();
        $data_sales = [];
        foreach ($sales as $sale)
        {
            $tmp_designation = $sale->Ref." / ".$sale->date." / ".$sale->GrandTotal;

            $data_sales[] = ['id'=> $sale->id, 'designation' => $tmp_designation];
        }
    ...
    public function get_warehoues()
    {
        $current_user = Auth::user();

        if ($current_user->warehouse !== 1 && $current_user->warehouse !== null)
            $warehouses = Warehouse::where('deleted_at', '=', null)->where('id', '<>', $current_user->warehouse)->get(['id', 'name']);
        else
            $warehouses = Warehouse::where('deleted_at', '=', null)->where('id', '<>', 1)->get(['id', 'name']);
    ...
    public function get_all_warehoues()
    {
        $warehouses = Warehouse::where('deleted_at', '=', null)->get(['id', 'name']);
