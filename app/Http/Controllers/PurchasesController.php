...
class PurchasesController extends BaseController
{
    ...
    public function All_Payments(Request $request)
    {
        ...
        $Filtred =  PaymentPurchase::with('purchase')
        ->where(function ($query) use ($view_records) {
            if (!$view_records) {
                return $query->where('user_id', '=', Auth::user()->id);
            }
    ...
    public function Purchase_pdf(Request $request, $id)
    {
        ...
        if ($Purchase_data->payment_statut === "unpaid")
            $purchase['payment_status_fr'] = 'Impayé';
        else if ($Purchase_data->payment_statut === "paid")
            $purchase['payment_status_fr'] = 'Payé';
        else
            $purchase['payment_status_fr'] = 'Partiel';
        
        if ($purchase['statut'] === "received")    
            $purchase['statut_fr'] = 'Reçu';
        else
            $purchase['statut_fr'] = 'En attente';
