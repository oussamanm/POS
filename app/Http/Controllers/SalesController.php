...
class SalesController extends BaseController
{
    ...
    public function index(request $request)
    {
            ...
            ->where(function ($query) use ($request) {
                return $query->when($request->filled('search'), function ($query) use ($request) {
                    return $query->where('Ref', 'LIKE', "%{$request->search}%")
                        ...
                        ->orWhere('payment_received', 'like', "%{$request->search}%")
        ...
        foreach ($Sales as $Sale) {
            ...
            $item['payment_received'] = $Sale['payment_received'];
    ...
    public function app_index(request $request)
    {
        ...
        if (!empty($request->search) && $request->search != "")
        {
            ...
            $Sales = $Sales->where(function($query) use ($search_value){
                $query->where('Ref', 'LIKE', "%{$search_value}%")
                    ->orWhere('GrandTotal', 'LIKE' ,"%{$search_value}%")
                    ->orWhere('client_name', 'LIKE' ,"%{$search_value}%");
            });
        }


        // Filter
        if (isset($request->checkUnpaidOrders) && $request->checkUnpaidOrders === "1")
        {
            $Sales = $Sales
                ->whereIn('payment_statut', ['unpaid', 'partiel'])
               ->whereDate('date', '>=', Carbon::now()->subDays(30));
        }
        else
        {
            if (isset($request->date) && !empty($request->date))
                $Sales = $Sales->where('date', $request->date);
    ...
    public function app_show($id)
    {
        ...
        foreach ($sale_data['details'] as $detail)
        {
            ...
            $data['price'] = $detail->price - $data['DiscountNet'];
            $data['Unit_price'] = $detail->price;
            $data['discount'] = $detail->discount;
            $data['image'] = $detail->product->image;
            $data['ar_name'] = $detail->product->ar_name;
            $data['prd_id'] = $detail->product->id;
            $data['id'] = $detail->id;

            $current_user_warehouse = Auth::user()->warehouse;
            if (empty($current_user_warehouse))
                $current_user_warehouse = 1;

            $data['stock'] = $this->getStockPrd($detail->product->id, $current_user_warehouse);
    ...
    public function getStockPrd($prd_id, $warehouse_id)
    {
        $current_stock = 0;

        $stock = product_warehouse::where('product_id', $prd_id)
            ->where('deleted_at', '=', null)
            ->where('warehouse_id', $warehouse_id)
            ->first();

        if (!empty($stock))
            $current_stock = $stock->qte;
    ...
    public function app_store(Request $request)
    {
        ...
        $rtn_sale = \DB::transaction(function () use ($request)
        {
            // Create Sale Details
            $order = new Sale;
            ...
            if ($request->payment['status'] != 'pending')
            {
                try{
                    ...
                    if($request['amount'] > 0)
                    {
                        // code renoved Paying Method credit card
        ...
        return response()->json(['success' => true, 'sale' => $rtn_sale]);
    }
    ...
    public function app_update(Request $request, $id)
    {
        ...
        if (SaleReturn::where('sale_id', $id)->where('deleted_at', '=', null)->exists())
            return response()->json(['success' => false , 'message' => 'Return exist for the Transaction'], 403);

        if ($current_Sale->payment_statut !== "unpaid")
            return response()->json(['success' => false , 'message' => 'Sale Already have Payments'], 403);

        try
        {
            \DB::transaction(function () use ($request, $id, $current_Sale)
            {
                $GrandTotal = 0;

                /// Check if Order details has updated
                ...
                // Debug from Postmen
                // $new_sale_details = json_decode($request['details'], true);
                // $new_sale_details = $request->json('details');
                $new_sale_details = $request['details']; // [{id: 1, qte: 1771}, ...]

                // Extract id's
                $ids = array_column($new_sale_details, 'id');

                foreach ($old_sale_details as $old_detail)
                {
                    // check if ID exist in new table and Update it
                    if (in_array($old_detail->id, $ids))
                    {
                        // find quantity
                        $new_quantity = 0;
                        foreach ($new_sale_details as $new_detail)
                        {
                            if ($new_detail['id'] === $old_detail->id)
                            {
                                $new_quantity = $new_detail['quantity'];
                                break ;
                            }
                        }
                        ...
                        $product_warehouse = product_warehouse::where('deleted_at', '=', null)
                            ...
                            ->where('product_id', $old_detail->product_id)
                            ->first();
                        if ($product_warehouse)
                        {
                            // Log::channel('myApiLog')->info('-----------------');
                            // Log::channel('myApiLog')->info('$product_warehouse->qte : '.$product_warehouse->qte);
                            // Log::channel('myApiLog')->info('$old_detail->quantity : '.$old_detail->quantity);
                            // Log::channel('myApiLog')->info('$new_quantity : '.$new_quantity);


                            $product_warehouse->qte = $product_warehouse->qte + $old_detail->quantity - $new_quantity;
                            $product_warehouse->save();
                        }
                        ...
                        // calculate tax
                        if ($old_detail->tax_method == '2')
                            $tmp_price = $old_detail->price;
                        else
                            $tmp_price = $old_detail->price + ($old_detail->price * $old_detail->TaxNet / 100);

                        // calculate discount
                        if ($old_detail->discount_method == '2')
                            $tmp_discount = $old_detail->discount;
                        else
                            $tmp_discount = $tmp_price * $old_detail->discount / 100;

                        $final_price = $tmp_price - $tmp_discount;

                        $subTotal = $final_price * $new_quantity;

                        $GrandTotal += $subTotal;

                        // update quantity and subtotal
                        SaleDetail::where('id', $old_detail->id)->update(['quantity' => $new_quantity, 'total' => $subTotal]);
                    }
                    else
                    {
                        //---- Else details was deleted

                        $SaleDetail = SaleDetail::findOrFail($old_detail->id);
                        $SaleDetail->delete();
                        ...
                        if ($product_warehouse)
                        {
                            $product_warehouse->qte += $old_detail->quantity;
                            $product_warehouse->save();
                        }

                        // update Total
                        $GrandTotal -= $old_detail->total;
                    }
                }
        ...
        \DB::transaction(function () use ($request, $id) {
            ...
            else
            {
                ...
                foreach ($old_sale_details as $key => $value)
                {
                    ...
                    // Delete Detail Sale if not exist
    ...
    public function app_destroy(Request $request, $id)
    {
        try
        {
            \DB::transaction(function () use ($id, $request)
            {
                ...
                if (SaleReturn::where('sale_id', $id)->where('deleted_at', '=', null)->exists())
                    throw new Exception('already_has_return');
                else
                {
                    foreach ($old_sale_details as $key => $value)
                    {
    ...
    public function app_Print_Invoice_POS($id)
    {
        $helpers = new helpers();
        $details = array();
        ...
        $item['GrandTotal'] = number_format($sale->GrandTotal, 2, '.', '');
        $item['paid_amount'] = number_format($sale->paid_amount, 2, '.', '');
        $item['reste'] = number_format(($sale->GrandTotal - $sale->paid_amount), 2, '.', '');
        ...
        foreach ($sale['details'] as $detail)
        {
            ...
            $data['price'] = number_format(($detail->total / $detail->quantity), 2, '.', '');
            $data['quantity'] = number_format($detail->quantity, 2, '.', '');
            $data['total'] = number_format($detail->total, 2, '.', '');
            $data['unit_sale'] = $unit?$unit->ShortName:'';
        ...
        if (empty($item) || empty($details))
            return response()->json(['error' => 'Invoice not found'], 404);


        // Render the Blade view to get HTML content
        $htmlContent = View::make('pdf.pos_print',
            [
                'symbol' => $symbol,
                'payments' => $payments,
                'setting' => $settings,
    ...
    public function Sale_PDF(Request $request, $id)
    {
        ...
        $sale['DocType'] = "Facture";

        $f = new \NumberFormatter( 'fr', \NumberFormatter::SPELLOUT );
        $word = $f->format($sale['GrandTotal']);

        $sale['GrandTotalText'] =strtoupper($word);
    ...
    public function My_Sale_PDF(Request $request, $id, $type)
    {
        ...
        $carbonDate = Carbon::parse($sale_data->date);
        ...
        $sale['due'] = number_format($sale['GrandTotal'] - $sale['paid_amount'], 2, '.', '');
        $sale['payment_status'] = $sale_data->payment_statut;
        $sale['DocType'] = ($type === "0") ? "Facture" : "Bon de Livraison";
    ...
    public function get_Products_by_sale(Request $request , $id)
    {
     ...
     public function pay_all_sales(Request $request)
     {
        // check if data exist

        // retrieve list sales with status != paid between data start and date fin
        $sales = Sale::select([''])
            ->where('payment_statut', '!=', 'paid')
            ->where('deleted_at', '=', null)
            ->whereBetween('date', [$startOfMonth, $endOfMonth]);

        // loop on sales id and call api payment
        ...
        foreach ($sales as $sale)
        {
            ...
            $this->store_payment_api($param);
        }

     }

    private function store_payment_api($request)
    {
        /*
        [
            'sale_id' => ,
