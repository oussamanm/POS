...
class ReportController extends BaseController
{
    ...
    public function getStatistic($id_warehouse)
    {

        $selected_warehouse = (empty($id_warehouse) || is_null($id_warehouse)) ? 1 : $id_warehouse;
        $current_user = Auth::user();

        $nbr_today_sales = 0;
        $total_today_sales = 0;
        $total_month_sales = 0;
        $total_credit_clients = 0;
        $total_payment_onHand = 0;
        ...
        /// Get sales Today
        $restult_nbr = Sale::where('date', $todaysDate)
            ->where('deleted_at', '=', null)
            ->where('warehouse_id', $selected_warehouse)
            ->when($current_user->role_id === 4, function ($query) use ($current_user) {
                return $query->where('user_id', $current_user->id);
            })
            ->count();

        if (!(is_null($restult_nbr) || empty($restult_nbr)))
            $nbr_today_sales = $restult_nbr;


        /// Get sales Today
        $restult_today_sales = Sale::where('date', $todaysDate)
            ->where('deleted_at', '=', null)
            ->where('warehouse_id', $selected_warehouse)
            ->when($current_user->role_id === 4, function ($query) use ($current_user) {
                return $query->where('user_id', $current_user->id);
        ...
        if (!(is_null($restult_today_sales) || empty($restult_today_sales)))
            $total_today_sales = $restult_today_sales;

        /// Get sales Month
        $restult_month_sales = Sale::whereBetween('date', [$startOfMonth, $endOfMonth])
            ->where('deleted_at', '=', null)
            ->where('warehouse_id', $selected_warehouse)
            ->when($current_user->role_id === 4, function ($query) use ($current_user) {
                return $query->where('user_id', $current_user->id);
        ...
        if (!(is_null($restult_month_sales) || empty($restult_month_sales)))
            $total_month_sales = $restult_month_sales;

        /// Get sold
        $credit_data = Sale::where('deleted_at', '=', null)
            ->where('warehouse_id', $selected_warehouse)
            ->selectRaw('SUM(GrandTotal) as total_sale, SUM(paid_amount) as total_paid')
            ->first();

        if (!(is_null($credit_data) || empty($credit_data)))
        {
            // get records returned
            $total_returns = SaleReturn::where('deleted_at', '=', null)
                ->where('warehouse_id', $selected_warehouse)
                ->where('payment_statut', '=', 'paid')
                ->sum('GrandTotal');

            $total_credit_clients = $credit_data->total_sale -  $credit_data->total_paid - $total_returns;
        }

        // Get Payments
        if ($current_user->role_id === 2)
        {
            $payment_data = PaymentSale::where('deleted_at', '=', null)->where('payment_received', '=', 0)->where('user_id', $current_user->id)->sum('montant');

            if (!(is_null($payment_data) || empty($payment_data)))
                $total_payment_onHand = $payment_data;
        }

        $data = [
            'nbr_today_sales' => $nbr_today_sales,
            'total_today_sales' => number_format($total_today_sales, 2, ',', ' '),
            'total_month_sales' => number_format($total_month_sales, 2, ',', ' '),
            'total_credit' => number_format($total_credit_clients, 2, ',', ' '),
            'total_payment_onHand' => number_format($total_payment_onHand, 2, ',', ' '),
        ];
    ...
    public function Client_Report(request $request)
    {
        ...
        {
            $sales = Sale::whereNull('deleted_at')->where('client_id', $client->id)->get();

            $item['total_sales'] = $sales->count();
            $item['total_amount'] = $sales->sum('GrandTotal');
            $item['total_payments'] = PaymentSale::whereNull('deleted_at')->where('client_id', $client->id)->where('sale_id', '!=', null)->sum('montant') ?? 0;
            // $item['total_return'] = SaleReturn::whereNull('deleted_at')->where('client_id', $client->id)->sum('GrandTotal') ?? 0;
            $item['total_return'] = PaymentSale::whereNull('deleted_at')->where('client_id', $client->id)->whereNull('sale_id')->sum('montant') ?? 0;

            $item['due'] = $item['total_amount'] - $item['total_payments'] - $item['total_return'];
    ...
    public function Client_Report_detail(request $request, $id)
    {
        ...
        $sales = Sale::where('deleted_at', '=', null)->where('client_id', $id)->get();

        $data['total_sales'] = $sales->count();
        $data['total_amount'] = $sales->sum('GrandTotal');
        $data['total_payments'] = PaymentSale::where('deleted_at', '=', null)->where('client_id', $id)->where('sale_id', '!=', null)->sum('montant') ?? 0;
        $data['total_return'] = SaleReturn::where('deleted_at', '=', null)->where('client_id', $id)->sum('GrandTotal') ?? 0;

        $data['due'] = $data['total_amount'] - $data['total_payments'] - $data['total_return'];
    ...
    public function Warehouse_Report(request $request)
    {
        ...
        if ($request->warehouse_id == null && isset($_GET['vendor']) && $_GET['vendor'] == 1)
            $request->warehouse_id = Warehouse::where('deleted_at', '=', null)->where('id', '>', '1')->first()->id;
        $data['warehouse_id'] = $request->warehouse_id;

        // Get Sales
        $data_sales = Sale::where('deleted_at', '=', null)
            ->where(function ($query) use ($request) {
        ...
        $data['sales'] = $data_sales->count();
        $tmp_sum_sales = $data_sales->sum('GrandTotal');
        $data['totalSales'] = number_format($tmp_sum_sales, 2, '.', ',');

        // Get Sales Return
        $data_sales_return = SaleReturn::where('deleted_at', '=', null)
            ->where(function ($query) use ($request) {
                    return $query->when($request->filled('from'), function ($query) use ($request) {
                        return $query->whereBetween('date', array($request->from, $request->to));
                });
        ...
        $data['ReturnSale'] = $data_sales_return->count();
        $tmp_sum_sales_return = $data_sales_return->sum('GrandTotal');
        $data['totalSalesReturn'] = number_format($tmp_sum_sales_return, 2, '.', ',');

        $data['reste'] = number_format($tmp_sum_sales - $tmp_sum_sales_return, 2, '.', ',');

        //get warehouses assigned to user
        $user_auth = auth()->user();
        if($user_auth->is_all_warehouses){
            $warehouses = Warehouse::where('active', 1)->where('deleted_at', '=', null);
            if (isset($_GET['vendor']) && $_GET['vendor'] == 1)
                $warehouses = $warehouses->where('id', '<>', 1);
            $warehouses = $warehouses->get(['id', 'name']);
        }else{
            $warehouses_id = UserWarehouse::where('user_id', $user_auth->id)->pluck('warehouse_id')->toArray();
            $warehouses = Warehouse::where('active', 1)->where('deleted_at', '=', null)->whereIn('id', $warehouses_id);
            if (isset($_GET['vendor']) && $_GET['vendor'] == 1)
                $warehouses = $warehouses->where('id', '<>', 1);
            $warehouses = $warehouses->get(['id', 'name']);
        }
    ...
    public function Pre_Vendeur_Report(request $request)
    {
        $this->authorizeForUser($request->user('api'), 'WarehouseStock', Product::class);

        $preVendeurs = User::
            whereHas('roles', function ($q) {
                $q->where('name', 'Pre-Vendeur');
            })
            ->get(['id', 'username']);

        // check if request has preVendeur_id selected or not
        if ($request->filled('preVendeur_id') && $request->preVendeur_id != "")
            $preVendeur_id = $request->preVendeur_id;
        else if ($preVendeurs->count() > 0)
            $preVendeur_id = $preVendeurs->first()->id;
        else
            $preVendeur_id = null;
        ...
        if ($preVendeur_id)
        {
            $data_sales = Sale::whereNull('deleted_at')
                ->when($request->filled('from'), function ($query) use ($request){
                        return $query->whereBetween('date', array($request->from, $request->to));
                })
                ->where('user_id', $preVendeur_id)
                ->get();

            $tmp_sum_sales = $data_sales->sum('GrandTotal') ?? 0;
            $data['sales'] = $data_sales->count() ?? 0;
            $data['totalSales'] = number_format($tmp_sum_sales, 2, '.', ',') ?? 0;
        }
    ...
    public function Sales_User(request $request)
    {
    ...
    public function Returns_Sale_User(request $request)
    {
        $this->authorizeForUser($request->user('api'), 'WarehouseStock', Product::class);
        // How many items do you want to display.
        $perPage = $request->limit;
    ...
    public function Warhouse_Count_Stock(Request $request)
    {
        ...
        foreach ($stock_value as $key => $value)
        {
            ...
            $item['value1'] = round($value->cost, 2);
            $data[] = $item;
        }
    ...
    // calculate sales total taxes (used to calculate sales HT)

    public function get_sales_ht($start_date, $end_date, $warehouse_id, $array_warehouses_id)
    {
        $sales_detail = SaleDetail::with('sale')
            ...
            ->selectRaw('sum((price/(1+ (TaxNet/100))) * quantity) as total_taxes')->first();

        if (is_null($sales_detail) || empty($sales_detail) || $sales_detail['total_taxes'] == null)
            return 0;
        return ($sales_detail['total_taxes']);
    }


    public function get_sales_return_tva($start_date, $end_date, $warehouse_id, $array_warehouses_id)
    {
        $sales_return = SaleReturnDetails::with('SaleReturn')
            ->where(function ($query) use ($warehouse_id, $array_warehouses_id, $start_date, $end_date) {
                if ($warehouse_id !== 0)
                {
                    return $query->whereHas('SaleReturn', function ($q) use ($array_warehouses_id, $warehouse_id, $start_date, $end_date) {
                        $q->where('warehouse_id', $warehouse_id)
                            ->where('deleted_at', '=', null)
            ...
            ->selectRaw('sum((TaxNet * price / 100) * quantity) as total_return_tva')->first();

        if (is_null($sales_return) || empty($sales_return) || $sales_return['total_return_tva'] == null)
            return 0;

        return $sales_return['total_return_tva'];
    }

    // calculate Purchases total taxes (used to calculate purchases HT)

    public function get_purchases_taxe($start_date, $end_date, $warehouse_id, $array_warehouses_id)
    {
        $detail = PurchaseDetail::with('purchase')
            ...
            ->selectRaw('sum(((TaxNet * cost)/100) * quantity) as total_taxes')->first();

        if (is_null($detail) || empty($detail) || $detail['total_taxes'] == null)
            return 0;

        return $detail['total_taxes'];
    }

    public function get_purchases_return_tva($start_date, $end_date, $warehouse_id, $array_warehouses_id)
    {
        $purchase_return = PurchaseReturnDetails::with('PurchaseReturn')
            ->where(function ($query) use ($warehouse_id, $array_warehouses_id, $start_date, $end_date) {
                if ($warehouse_id !== 0)
                {
                    return $query->whereHas('PurchaseReturn', function ($q) use ($array_warehouses_id, $warehouse_id, $start_date, $end_date) {
                        $q->where('warehouse_id', $warehouse_id)
                            ->where('deleted_at', '=', null)
                            ->where('statut', 'completed')
                            ->whereBetween('date', array($start_date, $end_date));
            ...
            ->selectRaw('sum((TaxNet * cost / 100) * quantity) as total_return_tva')->first();

        if (is_null($purchase_return) || empty($purchase_return) || $purchase_return['total_return_tva'] == null)
            return 0;

        return $purchase_return['total_return_tva'];    }

     //-----------------Profit And Loss ---------------------------\\

     public function old_with_ht_ProfitAndLoss(request $request)
     {
        ...
        // calculate sales HT and TVA

        $total_sales_ttc = $report_total_sales->sum;

        $total_sales_ht = $this->get_sales_ht($start_date, $end_date, $warehouse_id, $array_warehouses_id);
        $item['sale_ht'] = number_format($total_sales_ht, 2, '.', ',');

        $total_sales_tva = $total_sales_ttc - $total_sales_ht;

        $item['profit_fifo'] = number_format(($total_sales_ht - $cogs), 2, '.', ',');
        $item['profit_average_cost'] = number_format($total_sales_ht - $total_average_cost, 2, '.', ',');

        //-------------- Calculate TVA

        // get total sales en HT - sales return ht
        // $total_sales_return_tva = $this->get_sales_return_tva($start_date, $end_date, $warehouse_id, $array_warehouses_id);

        // $calcult_sale_ht = ($report_total_sales->sum - $total_sales_taxe) - ($report_total_returns_sales->sum - $total_sales_return_tva) ;

        // get total purchase en HT - purchase return ht
        // $total_purchase_taxe = $this->get_purchases_taxe($start_date, $end_date, $warehouse_id, $array_warehouses_id);
        $total_purchase_ttc = $report_total_purchases->sum ;

        $total_purchase_ht =  $total_purchase_ttc / 1.2;
        $total_purchase_tva = $total_purchase_ttc - $total_purchase_ht;


        // $total_purchase_return_tva = $this-> get_purchases_return_tva($start_date, $end_date, $warehouse_id, $array_warehouses_id);

        // $calcult_purchase_ht = ($report_total_purchases->sum - $total_purchase_taxe) - ($report_total_returns_purchases->sum - $total_purchase_return_tva);

        // $result_ht = $calcult_sale_ht - $calcult_purchase_ht;

        $item['tva_sales'] = number_format($total_sales_tva, 2, '.', ',');
        $item['tva_purchases'] = number_format($total_purchase_tva, 2, '.', ',');

        $item['tva_due'] = number_format(($total_sales_tva - $total_purchase_tva), 2, '.', ',');

        //-------------- Calculate tva_due

        // $item['sale_tva'] = number_format(($total_sales_taxe - $total_sales_return_tva), 2, '.', ',');
        // $item['purchase_tva'] = number_format(($total_purchase_taxe - $total_purchase_return_tva), 2, '.', ',');

        // $result_tva = ($total_sales_taxe - $total_sales_return_tva) - ($total_purchase_taxe - $total_purchase_return_tva);

        // $item['tva_due'] = number_format($result_tva, 2, '.', ',');
     ...
     public function old_CalculeCogsAndAverageCost($start_date, $end_date , $warehouse_id, $array_warehouses_id)
     {
            ...
            $average_cost = $average_cost/1.2;
             ...
             else
             {
                foreach ($sales as $sale)
                {
                    ...
                    /// achnahowa dawr dyal had l code ---> laho a3lam
                    while ($saleQuantity > 0)
                    {
                        ...
                        if ($purchase->quantity > 0)
                        {
                            ...
                            {
                                $totalCogs += $saleQuantity * ($purchase->cost / 1.2);
                            ...
                            else
                            {
                                ...
                                if($purchase->quantity > $diff)
                                {
                                    ...
                                    {
                                        $saleQuantity -= $rest;
                                        $totalCogs+= $rest * ($purchase->cost / 1.2);
                                        $totalQuantitySold =  0;
                                        $purchase->quantity = 0;
                                        $purchases->shift();
                                        ...
                                        $totalCogs+= $purchase->quantity * ($purchase->cost / 1.2);
     ...
     // its will calculate it from purchases and adjustments
     public function old_averageCost($product_id , $start_date, $end_date , $warehouse_id, $array_warehouses_id)
     {
         ...
         foreach ($purchases as $purchase)
         {
            ...
            // Log::channel('myApiLog')->info('################### $purchase->cost : '.$purchase->cost);
    ...
    public function stock_Report(request $request)
    {
        ...
        // if (isset($_GET['SortField']) && $_GET['SortField'] == "quantity")
        //     $order = "none";
        // else
        $order = $request->SortField;
        $dir = $request->SortType;
        ...
        foreach ($products as $product) {
            ...
            if($product->type != 'is_service')
            {
                ...
                $current_stock = product_warehouse::
                ...
                if (isset($_GET['vendor']) && $_GET['vendor'] == 1 && $current_stock == 0)
                {
                    $zeroQty += 1;
                    continue;
                }
                ...
                $item['stock_value'] = round($current_stock * $product->cost, 2);;
    ...
    public function product_report(request $request)
    {
        ...
        $products_data = Product::where('deleted_at', '=', null)
            ->with('category')
            ->select('id', 'name','code', 'is_variant','unit_id','type', 'category_id')
            ->where(function ($query) use ($request) {
                return $query->when($request->filled('search'), function ($query) use ($request) {
                    return $query->where('name','LIKE', "%{$request->search}%")
                        ->orWhere('code', 'LIKE', "%{$request->search}%");
        ...
        foreach ($products as $product) {
            ...
            if($product->type != 'is_service')
            {
                ...
                // if (isset($_GET['vendor']) && $sold_qty == 0)
                //     continue;
    ...
    // -------------------- api correct data : fill client_id in table payment_sales using sale_id -------------------\\

    public function correct_data_sales_payments()
    {
        $payments = PaymentSale::where('deleted_at', '=', null)->whereNull('client_id')->get();
