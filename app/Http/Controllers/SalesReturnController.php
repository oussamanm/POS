...
class SalesReturnController extends BaseController
{
    ...
    public function app_index(request $request)
    {
        ...
        // Check If User Has Permission View  All Records
        $SaleReturn = SaleReturn::with('sale', 'facture', 'client')
            ->where('deleted_at', '=', null)
            ->where(function ($query) use ($view_records, $role) {
                if (!$view_records || $role->id !== 1)
                    return $query->where('user_id', '=', Auth::user()->id);
            });
        ...
        if (!empty($request->search) && $request->search !== "")
        {
            ...
            $SaleReturn = $SaleReturn->where(function ($query) use ($search_value){
                $query->where('Ref', 'LIKE', "%{$search_value}%")
                    ->orWhere('GrandTotal', 'LIKE' ,"%{$search_value}%")
                    ->orWhereHas('client', function ($query) use ($search_value) {
        ...
        if (isset($request->date) && !empty($request->date))
            $SaleReturn = $SaleReturn->where('date', $request->date);
    ...
    public function getNumberOrderPayment()
    {
        ...
        \DB::transaction(function () use ($request, $id) {
            ...
            foreach ($new_return_details as $key => $product_detail) {
                ...
                if($product_detail['no_unit'] !== 0 || $get_type_product == 'is_service'){
                    ...
                    $orderDetails['price'] = $product_detail['price'];
    ...
    public function update(Request $request, $id)
    {
        ...
        if (isset($request['new']))
            return self::new_update($request, $id);
        \DB::transaction(function () use ($request, $id) {
            $role = Auth::user()->roles()->first();
            $view_records = Role::findOrFail($role->id)->inRole('record_view');
            $current_SaleReturn = SaleReturn::findOrFail($id);
    ...
    public function app_show(Request $request, $id)
    {
    ...
    public function create_sell_return(Request $request , $id)
    {
        ...
        if ($id == 'new')
        {
            return self::new_create_sell_return($request);
        }
        $role = Auth::user()->roles()->first();
        $view_records = Role::findOrFail($role->id)->inRole('record_view');
    ...
    public function new_edit_sell_return(Request $request, $id){
    ...
    public function edit_sell_return(Request $request, $id, $sale_id)
    {
        ...
        $this->authorizeForUser($request->user('api'), 'update', SaleReturn::class);
        if ($sale_id == 'null')
            return self::new_edit_sell_return($request, $id);
        $role = Auth::user()->roles()->first();
        $view_records = Role::findOrFail($role->id)->inRole('record_view');
        $SaleReturn = SaleReturn::with('sale','details.product.unitSale')
            ->where('deleted_at', '=', null)
