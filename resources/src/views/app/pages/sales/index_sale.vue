          ...
          <span v-if="props.column.field == 'actions'">
            <div>
              ...
              >
                ...
                <b-dropdown-item title="PDF" @click="Invoice_PDF(props.row , props.row.id, 0)">
                  <i class="nav-icon i-File-TXT font-weight-bold mr-2"></i>
                  Télécharger Facture
                </b-dropdown-item>
      ...
      <div class="px-3 py-2">
        <b-row>
          ...
          <b-col md="12">
           ...
           <!-- Payment recu Status  -->
          <b-col md="12">
            <b-form-group :label="$t('payment_received')">
              <v-select
                v-model="Filter_Payment_recieved"
                :reduce="label => label.value"
                :placeholder="$t('Choose_Status')"
                :options="
                      [
...
export default {
  ...
  methods: {
    ...
    //-----------------------------  Invoice PDF ------------------------------\\
    Invoice_PDF(sale, id, type)
    {
      // Start the progress bar.
      NProgress.start();
      NProgress.set(0.1);
       axios
        .get("my_sale_pdf/" + id + "/"+type, {
