<template>
  <div class="main-content">
    <breadcumb :page="$t('Affectation de stock')" :folder="$t('Affectation de stock')"/>
      ...
      >
        ...
        <template slot="table-row" slot-scope="props">
          <span v-if="props.column.field == 'actions'">
            <a title="Download" v-b-tooltip.hover @click="Details_PDF(props.row.id)">
            ...
            <router-link
              ...
              title="Edit"
              v-b-tooltip.hover
              :to="{ name:'edit_stock_assignment', params: { id: props.row.id } }"
            >
...
export default {
  ...
  methods: {
    ...
    //     let de = response.data.details;
    //     let re = response.data.transfer.Ref;
    //     let date = response.data.transfer.date; // Assuming there's a 'date' property in the transfer object
    //     let from = response.data.transfer.from_warehouse; // Assuming there's a 'date' property in the transfer object
    //     let to = response.data.transfer.to_warehouse; // Assuming there's a 'date' property in the transfer object
    ...
    //       ];
    //       pdf.autoTable(columns, de, { startY: 110 });
          
    //       pdf.text("Affectation de stock ("+re+")", 40, 45);
    //       pdf.setFontSize(12)
    //       pdf.text("Date: " + date, 40, 65); // Adjust the vertical position as needed
    //       pdf.text("Transfert De :" + from, 40, 80); // Adjust the vertical position as needed
    //       pdf.text("Transfert À :" + to, 40, 95); // Adjust the vertical position as needed

    //       pdf.save("details-"+re+".pdf");
    //       NProgress.done()
    //     })
    //     .catch(response => {
...
Details_PDF(id) {
  ...
  axios
    ...
    .then(response => {
      let de = response.data.details;
      let re = response.data.transfer.Ref;
      let date = response.data.transfer.date;
      let from = response.data.transfer.from_warehouse;
      let to = response.data.transfer.to_warehouse;
      ...
      // Check if 'de' is an array and not empty
      if (Array.isArray(de) && de.length > 0) {
        // Add event listener for the `didDrawPage` event
        pdf.autoTable({
          head: [columns],
          body: de.map(item => [item.code, item.name, item.quantity]), // Map data to array format expected by autoTable
          startY: 110,
          didDrawPage: function (data) {
            ...
            if (data.pageNumber === 1) {
              ...
              pdf.text("Affectation de stock (" + re + ")", 40, 45);
              pdf.setFontSize(12);
              pdf.text("Date: " + date, 40, 65);
              pdf.text("Transfert De: " + from, 40, 80);
              pdf.text("Transfert À: " + to, 40, 95);
            }
          }
        });
      }
    ...
    Transfer_PDF() {
      ...
      let columns = [
        ...
        { title: "À partir de", dataKey: "from_warehouse" },
        { title: "Vert", dataKey: "to_warehouse" },
        { title: ("Total produits"), dataKey: "items" },
        { title: this.$t("Status"), dataKey: "statut" }
      ];
      pdf.autoTable(columns, self.transfers);
      pdf.text("Affectation de stock", 40, 25);
