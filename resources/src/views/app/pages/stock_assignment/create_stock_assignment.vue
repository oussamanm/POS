<template>
  <div class="main-content">
    <breadcumb :page="$t('Créer une affectation de stock')" :folder="$t('Affectation de stock')"/>
          ...
          <b-col lg="12" md="12" sm="12">
            <b-card>
              <b-row>
                ...
                <b-col lg="4" md="4" sm="12" class="mb-3">
                  <validation-provider name="the_path" :rules="{ required: true}">
                    <b-form-group slot-scope="{ valid, errors }" :label="$t('Ajouter / Récupérer le stock des vendeur') + ' ' + '*'">
                      <v-select
                        :class="{'is-invalid': !!errors.length}"
                        :state="errors[0] ? false : (valid ? true : null)"
                        v-model="the_path"
                        @input="Selected_the_path"
                        :reduce="label => label.value"
                        :options="
                                [{label: 'Ajouter le stock', value: '1'},
                                {label: 'Récupérer le stock', value: '2'}]"
                      ></v-select>
                ...
                <!-- From warehouse -->
                <b-col v-if="the_path != 1" lg="4" md="4" sm="12" class="mb-3">
                  <validation-provider name="From" :rules="{ required: true}">
                    <b-form-group slot-scope="{ valid, errors }" :label="'À partir de' + ' ' + '*'">
                      <v-select
                        :class="{'is-invalid': !!errors.length}"
                        :state="errors[0] ? false : (valid ? true : null)"
                ...
                <!-- To warehouse -->
                <b-col v-if="the_path != 2" lg="4" md="4" sm="12" class="mb-3">
                  <validation-provider name="To Warehouse" :rules="{ required: true}">
                    <b-form-group slot-scope="{ valid, errors }" :label="'Vers ' + '*'">
                      <v-select
                        :class="{'is-invalid': !!errors.length}"
                        :state="errors[0] ? false : (valid ? true : null)"
                ...
                <!-- Order Tax  -->
                <b-col lg="4" md="4" sm="12" class="mb-3 d-none">
                  <validation-provider
                    name="Order Tax"
                    :rules="{ regex: /^\d*\.?\d*$/}"
                    v-slot="validationContext"
              ...
              <validation-provider name="Tax Method" :rules="{ required: true}">
                <b-form-group slot-scope="{ valid, errors }" :label="$t('TaxMethod') + ' ' + '*'">
                  <v-select
                    ...
                    :options="
                           [
                            {label: $t('Exclusive'), value: '1'},
                            {label: $t('Inclusive'), value: '2'}
                           ]"
                  ></v-select>
                  <b-form-invalid-feedback>{{ errors[0] }}</b-form-invalid-feedback>
                </b-form-group>
...
export default {
  ...
  methods: {
    ...
    Selected_the_path(value) {
        ...
        if (value == 1)
        {
            //Add (Just Main warehouse)
            this.the_from = this.warehouses.filter((warehouses) => warehouses.id == 1);
            this.the_from = this.the_from.map(warehouses => ({label: warehouses.name, value: warehouses.id}))
            this.transfer.from_warehouse = 1;
            this.Selected_From_Warehouse(1)
            this.the_to = this.warehouses.filter((warehouses) => warehouses.id != 1);
            this.the_to = this.the_to.map(warehouses => ({label: warehouses.name, value: warehouses.id}))
