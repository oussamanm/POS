<template>
  <div class="main-content">
    <breadcumb page="Rapport des vendeurs" :folder="$t('Reports')"/>
    ...
    <b-row v-if="!isLoading">
      ...
      <!-- <b-col lg="3" md="6" sm="12" class="d-none">
        <b-card class="card-icon-bg card-icon-bg-primary o-hidden mb-30 text-center">
      ...
      <b-col lg="3" md="6" sm="12">
        <b-card class="card-icon-bg card-icon-bg-primary o-hidden mb-30 text-center">
          <i class="i-Turn-Right"></i>
          <div class="content">
            <p class="text-muted mt-2 mb-0">Total Retour des ventes ({{total.ReturnSale}})</p>
            <p class="text-primary text-24 line-height-1 mb-2">{{total.totalSalesReturn}}</p>
          </div>
        </b-card>
      </b-col>
      ...
      <b-col lg="3" md="6" sm="12">
        <b-card class="card-icon-bg card-icon-bg-primary o-hidden mb-30 text-center">
          <i class="i-Checkout-Basket"></i>
          <div class="content">
            <p class="text-muted mt-2 mb-0">Reste</p>
            <p class="text-primary text-24 line-height-1 mb-2">{{total.reste}}</p>
          </div>
        </b-card>
      </b-col>
      ...
      <b-col md="12">
        <b-card no-body class="card mb-30" header-bg-variant="transparent ">
          <b-tabs active-nav-item-class="nav nav-tabs" content-class="mt-3">
            ...
            <!-- Payments Table -->
            <b-tab :title="'Paiements à Encaisser'">
              <vue-good-table
                mode="remote"
                ...
                @on-selected-rows-change="selectionChangedPayments"

                styleClass="order-table vgt-table mt-2"
              >
                <div slot="selected-row-actions">
                    <button class="btn btn-primary btn-sm" @click="set_recieved_by_selected()">Paiement Reçu</button>
                </div>
              ...
              <div style="text-align:right;margin-top:15px"><b style="color:green">Total Sommaire :</b> {{totalSummaryPayments}} DH</div>
            ...
            <b-tab title="Stock Actuel">
              <vue-good-table
                mode="remote"
                :columns="columns_Stock"
                :totalRows="totalRows_Stock"
                :rows="Stock"
                @on-page-change="PageChangeStock"
                @on-per-page-change="onPerPageChangeStock"
                :pagination-options="{
                    enabled: false,
                    mode: 'records',
                    nextLabel: 'next',
              ...
              >
               <div slot="table-actions" class="mt-2 mb-3">
                <b-button @click="Stock_PDF()" size="sm" variant="outline-success ripple m-1">
                  <i class="i-File-Copy"></i> PDF
                </b-button>
              </div>
              </vue-good-table>
  ...
  methods: {
    ...
    Payments_PDF() {
      ...
      let columns = [
        ...
        { title: "Montant", dataKey: "total_payment" },
      ];
      pdf.autoTable(columns, self.payments);

      pdf.text("Liste des Paiements à Encaisser", 40, 25);
        ...
        pdf.setFontSize(15);
        pdf.text(`Total des Paiements : ${totalAmount} DH`, 40, pdf.autoTable.previous.finalY + 50); // Adjust the Y position as needed


        let date_now = moment().format('DD-MM-YYYY');
        pdf.save("Liste_Paiements_Encaisser_"+date_now+".pdf");
    },
    ...
    Stock_PDF() {
      ...
      let pdf = new jsPDF("p", "pt");
      let columns = [
        { title: this.$t("ProductCode"), dataKey: "code" },
        { title: this.$t("Name_product"), dataKey: "name" },
        // { title:  this.$t("Categorie"), dataKey: "category" },
        { title: this.$t("Current_stock"), dataKey: "quantity" },
      ];
      pdf.autoTable(columns, self.Stock);
      pdf.text("Stock Actuel", 40, 25);
      pdf.save("Stock.pdf");
    },
    ...
    Get_Stock(page){
        axios
        .get("report/stock?vendor=1&SortField=id&SortType=desc&page=" + page +
            "&limit=-1&warehouse_id=" +
            this.Filter_warehouse +
            "&search="+this.search_stock
        )
        .then(response => {
          this.totalRows_Stock = response.data.totalRows;
          this.Stock = response.data.report;
          setTimeout(() => {
            this.isLoading = false;
          }, 500);
        })
    ...
    Get_Reports() {
      axios
        .get("report/warehouse_report?vendor=1&warehouse_id=" + this.Filter_warehouse +"&to=" +this.endDate +"&from=" +this.startDate)
        .then(response => {
          this.total = response.data.data;
          if (response.data.warehouses.length)
          {
            this.warehouses = response.data.warehouses
            if (this.Filter_warehouse == "")
            {
                this.Filter_warehouse = response.data.data.warehouse_id;
    ...
    //--------------------------- Get Payments By warehouse -------------\\
    Get_Payments() {

      axios.get(
          "report/payments_vendeur?warehouse_id=" + this.Filter_warehouse + "&to=" + this.endDate + "&from=" + this.startDate
        )
        .then(response => {
          this.payments = response.data.payments;
          this.totalSummaryPayments = response.data.totalSummary;
    ...
    onPerPageChangeStock({ currentPerPage }) {
      if (this.limit_stock !== currentPerPage) {
        this.limit_stock = currentPerPage;
        this.Get_Stock(1);
      }
    },
    ...
    onSearch_Stock(value){
        this.search_stock = value.searchTerm;
        this.Get_Stock(1);
    },
    ...
    //---- Event Select Rows Payments
    selectionChangedPayments({ selectedRows })
    {
      this.selectedRef = [];
      selectedRows.forEach((row, index) => {
        this.selectedRef.push(row.Ref);
      });
    },
    ...
    //---- Set Payment Received  oussama
    set_recieved_by_selected() {

        const tab_ref = [];

        this.selectedRef.forEach((row) => {
            tab_ref.push(row);
        });

        this.Payments_PDF();

        const refString = tab_ref.join(',');

        axios
        .post("report/recieved_payments", { tab_ref })
        .then(response => {
          ...
          else
          {
            this.makeToast(
                "warning",
                "l'opération a échoué :"+response.data.message,
                this.$t("Warning")
            );
          }
    ...
    Submit_filter_dateRange() {
      ...
      console.log("+++++++++++ self.dateRange.startDate : ", self.dateRange.startDate);
      console.log("+++++++++++ self.dateRange.endDate : ", self.dateRange.endDate);
    ...
    get_date_loaded() {
      ...
      if (self.today_mode)
      {
        ...
        console.log("----- in get_date_loaded with startDate : ", self.startDate);
