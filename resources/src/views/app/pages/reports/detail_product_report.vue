  ...
  methods: {
    ...
    Sales_PDF() {
      ...
      let columns = [
        ...
        { title: this.$t("Created_by"), dataKey: "created_by" },
        { title: this.$t("product_name"), dataKey: "product_name" },
        { title: this.$t("Client"), dataKey: "client_name" },
        { title: this.$t("Warehouse"), dataKey: "warehouse_name" },
        { title: this.$t("Quantity"), dataKey: "quantity" },
        { title: this.$t("SubTotal"), dataKey: "total" },
      ];
      
    // Calculate the total amount
      let totalAmount = self.sales.reduce((total, sale) => {
    ...
    pdf.setFontSize(15);
    pdf.text(`Total des Ventes: ${totalAmount} DH`, 40, pdf.autoTable.previous.finalY + 50); // Adjust the Y position as needed
      pdf.save("Liste_des_ventes.pdf");
    },
