...
export default {
  ...
  methods: {
    ...
    if (this.warehouse_id != "")
    {
        ...
        const tmp_warehouse = this.warehouses.find(item => item.id == tmp);

        selected_depot = tmp_warehouse.name;
    }
    else
        selected_depot = "Tous les Entrepôts";
    ...
    if (Array.isArray(self.reports) && self.reports.length > 0)
    {
        ...
        pdf.autoTable({
            head: [columns],
            body: self.reports.map(item => [item.code, item.name, item.quantity, item.stock_value ]), // Map data to array format expected by autoTable
            startY: 110,
            didDrawPage: function (data) {
                ...
                pdf.text(`Page ${data.pageNumber}`, data.settings.margin.left, footerPosition);
            }
        });
    }
