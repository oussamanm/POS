...
export default {
  ...
  methods: {
    ...
    const modifiedSalesData = self.purchases.map(purchase => {
        ...
        if (purchase.payment_status === 'paid')
            ...
            paymentStatusValue='Non payé';
        else if (purchase.payment_status === 'partial')
            paymentStatusValue='Partiel';

        if (purchase.statut === 'received')
            purchase_status='Reçu';
        else if (purchase.statut === 'pending')
            purchase_status='En attente';
        else if (purchase.statut === 'Ordered')
            purchase_status='Commandé';
            
        return {
            ...purchase,
            payment_status: paymentStatusValue,
            statut: purchase_status,
        };
    });
      ...
      let columns = [
        ...
        { title: this.$t("Provider"), dataKey: "provider_name" },
      ...
      pdf.autoTable(columns, modifiedSalesData);
      pdf.text("Liste des achats", 40, 25);
      pdf.save("Liste_des_achats.pdf");
    },
