<template>
    <div class="main-content">
        ...
        <b-card class="wrapper" v-if="!isLoading">
            <vue-good-table mode="remote" :columns="columns" :totalRows="totalRows" :rows="stocks"
            ...
            >
                <div slot="table-actions" class="mt-2 mb-3">
                    <b-button @click="New_count()" class="btn-rounded" variant="btn btn-primary btn-icon m-1">
                        <i class="i-Add"></i>
                        Add New
                    </b-button>
                </div>
                ...
                <template slot="table-row" slot-scope="props">
                    <span v-if="props.column.field == 'actions'">
                      ...
                      <a v-if="!props.row.applied" v-b-tooltip.hover title="Applique stock" class="cursor-pointer" @click="Apply_stock(props.row.id)">
                        <i class="i-Yes text-25 text-info"></i>
                      </a>
                      <router-link
                        v-b-tooltip.hover
                        title="Edit"
                        :to="'/app/inventory/edit/'+props.row.id"
                      >
                            ...
                            <validation-provider name="date" :rules="{ required: true }" v-slot="validationContext">
                                <b-form-group :label="$t('date') + ' ' + '*'">
                                    <b-form-input :state="getValidationState(validationContext)"
                                        aria-describedby="date-feedback" type="date"
                                        v-model="stock.date"></b-form-input>
                            ...
                            <validation-provider name="warehouse" :rules="{ required: true }">
                                <b-form-group slot-scope="{ valid, errors }" :label="$t('warehouse') + ' ' + '*'">
                                    <v-select :class="{ 'is-invalid': !!errors.length }"
                                        :state="errors[0] ? false : (valid ? true : null)" @input="Selected_Warehouse"
                                        v-model="stock.warehouse_id" :reduce="label => label.value"
    ...
    methods: {
        ...
        Apply_stock(id) {
            ...
            this.$swal({
                ...
                confirmButtonText: "Oui, Appliquer le Stock"
            }).then(result => {
                if (result.value) {
                    // Start the progress bar.
                    NProgress.start();
                    NProgress.set(0.1);
                    axios
                        .get("inventory/" + id + "/apply_stock")
                        .then(() => {
                            this.$swal(
                                "Opération appliquée avec succès",
                                "Nouveau Stock est Appliqué",
                                "success"
                            );
