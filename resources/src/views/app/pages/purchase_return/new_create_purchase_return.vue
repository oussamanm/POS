              ...
              <b-row>
                ...
                <b-col lg="4" md="4" sm="12" class="mb-3">
                  <validation-provider name="warehouse" :rules="{ required: true}">
                    <b-form-group slot-scope="{ valid, errors }" :label="$t('warehouse') + ' & vendeurs ' + '*'">
                      <v-select
                        :class="{'is-invalid': !!errors.length}"
                        :state="errors[0] ? false : (valid ? true : null)"
                    ...
                    <table class="table table-hover">
                      ...
                      <tbody>
                        ...
                        <tr v-for="detail  in details">
                          ...
                          <td>{{currentUser.currency}} {{formatNumber(detail.quantity * detail.price, 3)}}</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
...
export default {
  ...
  methods: {
    ...
    //--- Submit Validate Create Purchase Return
    ...
    Verified_Qty(detail, id) {
        ...
        if (detail.quantity <= 0) {
            this.makeToast("warning", "La quantité ne peut pas être négative", this.$t("Warning"));
    ...
    increment(detail, id) {
      ...
      this.$forceUpdate(detail.quantity);
      this.Calcul_Total();
    },
    ...
    Calcul_Total() {
      ...
      for (var i = 0; i < this.details.length; i++) {
        this.details[i].subtotal = parseFloat(
          this.details[i].quantity * this.details[i].price
        );
    ...
    Get_Product_Details(product_id) {
      axios.get("/show_product_data/" + product_id +"/null").then(response => {
        ...
        this.product.price = response.data.Total_price;
    ...
    SearchProduct(result) {
      ...
      if (this.details.length > 0 && this.details.some(detail => detail.code === result.code)) {
        ...
        this.product.stock = 1000000;
        this.product.fix_stock = 1000000;
      ...
      if (this.verifiedForm()) {
        ...
        axios
          ...
          .then(response => {
            ...
            this.$router.push({ name: "index_purchase_return" });
          })
          .catch(error => {
            NProgress.done();
    ...
    Get_Products() {
      // Start the progress bar.
        ...
        NProgress.set(0.1);
      axios
        .get("get_products_return?type_return=purchase")
         .then(response => {
